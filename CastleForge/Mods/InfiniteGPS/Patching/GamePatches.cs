...
namespace InfiniteGPS
{
    ...
    class GamePatches
    {
        ...
        /// <summary>
        /// Problem:
        ///   All GPS variants (normal GPS, TeleportGPS, future GPS types) can lose durability
        ///   when <see cref="InventoryItem.InflictDamage"/> is called, and eventually break.
        ///
        /// Fix:
        ///   Intercept InventoryItem.InflictDamage() and, if the instance is any
        ///   <see cref="GPSItem"/>, always report "no break" and skip the original logic.
        ///
        /// Summary (BEFORE -> AFTER):
        ///   • BEFORE: GPS / TeleportGPS / other GPS variants can be consumed after enough uses.
        ///   • AFTER : All GPSItem-based items effectively have infinite durability and never break.
        /// </summary>
        [HarmonyPatch(typeof(InventoryItem), "InflictDamage")]
        static class Patch_InventoryItem_InflictDamage_InfiniteGPS
        {
            /// <summary>
            /// Prefix: Runs before the original InflictDamage().
            /// If this item is any GPSItem, short-circuits the call and pretends it never breaks.
            /// </summary>
            static bool Prefix(InventoryItem __instance, ref bool __result)
            {
                // Only special-case GPS items; everything else keeps vanilla behavior.
                if (!(__instance is GPSItem))
                    return true; // Not a GPSItem -> let the original InflictDamage run.

                // For all GPSItem variants:
                // Tell the caller "item did not break", and skip the base durability logic.
                __result = false;
                return false;    // Skip original InflictDamage().
            }
        }
        #endregion
    }
