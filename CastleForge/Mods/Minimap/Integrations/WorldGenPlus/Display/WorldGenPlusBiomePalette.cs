...
namespace Minimap.Integrations.WorldGenPlus
{
    /// <summary>
    /// Biome display helpers for Minimap (colors + friendly names).
    ...
    internal static class WorldGenPlusBiomePalette
    {
        ...
        public static string ShortBiomeName(string biomeType)
        {
            ...
            string shortName = (dot >= 0 && dot + 1 < biomeType.Length)
                ? biomeType.Substring(dot + 1)
                : biomeType;

            if (shortName.EndsWith("Biome", StringComparison.OrdinalIgnoreCase))
                shortName = shortName.Substring(0, shortName.Length - "Biome".Length);
        ...
        {
            if (c == MinimapConfig.ClassicBiomeColor)  return "Classic";
            if (c == MinimapConfig.LagoonBiomeColor)   return "Lagoon";
            if (c == MinimapConfig.DesertBiomeColor)   return "Desert";
            if (c == MinimapConfig.MountainBiomeColor) return "Mountain";
            if (c == MinimapConfig.ArcticBiomeColor)   return "Arctic";
            if (c == MinimapConfig.DecentBiomeColor)   return "Decent";
            if (c == MinimapConfig.HellBiomeColor)     return "Hell";
            return "Custom";
        }
        #endregion
