...
namespace VeinMiner
{
    ...
    class GamePatches
    {
        ...
        /// USAGE
        /// -----
        /// VMHotkeys.SetReloadBinding("Ctrl+Shift+F3");
        /// // ... each frame (via Harmony patch) -> if (VMHotkeys.ReloadPressedThisFrame()) { VMConfig.LoadApply(); ... }
        ///
        /// EXAMPLES
        /// --------
        /// "F9"                 -> F9.
        ...
        /// </summary>
        internal static class VMHotkeys
        {
            ...
            public static void SetReloadBinding(string s)
            {
                ...
                Log($"[VMiner] Reload hotkey set to \"{s}\".");
            }
        ...
        static class Patch_Hotkey_ReloadConfig_WorldEdit
        {
            ...
            {
                if (!VMHotkeys.ReloadPressedThisFrame()) return;
                ...
                try
                {
                    ...
                    SendFeedback($"[VMiner] Config hot-reloaded from \"{PathShortener.ShortenForLog(VMConfig.ConfigPath)}\".");
                }
                catch (Exception ex)
                {
                    SendFeedback($"[VMiner] Hot-reload failed: {ex.Message}.");
                }
            }
        }
        #endregion
        ...
        /// <summary>
        /// Captures the exact ore block the player began digging so the postfix can evaluate it
        /// after vanilla removes the original block.
        /// </summary>
        private struct DigContext
        {
            public bool          Valid;
            public IntVector3    Target;
            public BlockTypeEnum TargetType;
        }

        /// <summary>
        /// Triggers vein mining only after an effective pick dig against a vein-minable ore block.
        /// </summary>
        [HarmonyPatch(typeof(InGameHUD), nameof(InGameHUD.Dig))]
        internal static class Patch_InGameHUD_Dig_VeinMiner
        {
            static void Prefix(InGameHUD __instance, InventoryItem tool, bool effective, ref DigContext __state)
            {
                __state = default;

                try
                {
                    if (!effective || tool == null)
                        return;

                    if (!VeinMinerSystem.IsVeinMinerTool(tool))
                        return;

                    if (__instance == null || __instance.ConstructionProbe == null || !__instance.ConstructionProbe._collides)
                        return;

                    IntVector3    target     = __instance.ConstructionProbe._worldIndex;
                    BlockTypeEnum targetType = InGameHUD.GetBlock(target);
                    if (!VeinMinerSystem.IsVeinMinable(targetType))
                        return;

                    __state.Valid      = true;
                    __state.Target     = target;
                    __state.TargetType = targetType;
                }
                catch (Exception ex)
                {
                    Log($"Dig prefix failed: {ex.Message}.");
                }
            }

            static void Postfix(InGameHUD __instance, InventoryItem tool, bool effective, DigContext __state)
            {
                try
                {
                    if (!effective || !__state.Valid || tool == null || __instance == null || __instance.LocalPlayer == null)
                        return;

                    if (!(__instance.LocalPlayer.Gamer is LocalNetworkGamer localGamer))
                        return;

                    VeinMinerSystem.TryMineVein(__state.Target, __state.TargetType, tool, localGamer);
                }
                catch (Exception ex)
                {
                    Log($"Dig postfix failed: {ex.Message}.");
                }
            }
        }
        #endregion
