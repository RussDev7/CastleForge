...
namespace PhysicsEngine
{
    ...
    class GamePatches
    {
        ...
        /// USAGE
        /// -----
        /// PEHotkeys.SetReloadBinding("Ctrl+Shift+F3");
        /// // ... each frame (via Harmony patch) -> if (PEHotkeys.ReloadPressedThisFrame()) { PEConfig.LoadApply(); ... }
        ///
        /// EXAMPLES
        /// --------
        /// "F9"                 -> F9.
        ...
        /// </summary>
        internal static class PEHotkeys
        {
            ...
            public static void SetReloadBinding(string s)
            {
                ...
                Log($"[TFeller] Reload hotkey set to \"{s}\".");
            }
        ...
        static class Patch_Hotkey_ReloadConfig_WorldEdit
        {
            ...
            {
                if (!PEHotkeys.ReloadPressedThisFrame()) return;
                ...
                try
                {
                    ...
                    SendFeedback($"[PEngine] Config hot-reloaded from \"{PathShortener.ShortenForLog(PEConfig.ConfigPath)}\".");
                }
                catch (Exception ex)
                {
                    SendFeedback($"[TFeller] Hot-reload failed: {ex.Message}.");
                }
            }
        }
        #endregion
        ...
        #region Placement / Replication Hooks

        /// <summary>
        /// Local authoritative placement hook.
        ///
        /// This catches the placing player's own lava placement early so single-player
        /// and host gameplay register a source immediately. The actual terrain write is
        /// still left to vanilla / the network path.
        /// </summary>
        [HarmonyPatch(typeof(BlockInventoryItem), nameof(BlockInventoryItem.AlterBlock))]
        internal static class Patch_BlockInventoryItem_AlterBlock_LavaSource
        {
            static void Postfix(BlockInventoryItem __instance, Player player, IntVector3 addSpot, BlockFace inFace)
            {
                try
                {
                    if (!LavaPhysics.CanDriveSimulation())
                        return;

                    BlockTypeEnum placedType = __instance.GetConstructedBlockType(inFace, addSpot);
                    if (!LavaPhysics.IsLava(placedType))
                        return;

                    LavaPhysics.RegisterManualSource(addSpot, placedType);
                }
                catch (Exception ex)
                {
                    Log($"AlterBlock postfix failed: {ex.Message}");
                }
            }
        }

        /// <summary>
        /// Authoritative terrain-change observer.
        ///
        /// Why this patch exists:
        /// - Remote players do not hit our local item hook on the host.
        /// - All terrain block changes pass through this method.
        /// - We use it to adopt remote lava placements as sources and to detect when
        ///   source / simulated lava cells were manually removed or overwritten.
        /// </summary>
        [HarmonyPatch(typeof(CastleMinerZGame), "_processAlterBlocksMessage")]
        internal static class Patch_CastleMinerZGame_ProcessAlterBlocksMessage_PhysicsObserver
        {
            static void Postfix(Message message)
            {
                try
                {
                    ...
                    LavaPhysics.OnObservedBlockChange(abm.BlockLocation, abm.BlockType);
                }
                catch (Exception ex)
                {
                    Log($"AlterBlocks observer failed: {ex.Message}");
                }
            }
        }
        #endregion
