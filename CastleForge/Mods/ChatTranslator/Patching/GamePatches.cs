...
namespace ChatTranslator
{
    ...
    class GamePatches
    {
        ...
        /// USAGE
        /// -----
        /// CTHotkeys.SetReloadBinding("Ctrl+Shift+F3");
        /// // ... each frame (via Harmony patch) -> if (WEHotkeys.ReloadPressedThisFrame()) { CTConfig.LoadApply(); ... }
        ///
        /// EXAMPLES
        /// --------
        /// "F9"                 -> F9.
        ...
        /// </summary>
        internal static class CTHotkeys
        {
            ...
            public static void SetReloadBinding(string s)
            {
                ...
                Log($"[CTChat] Reload hotkey set to \"{s}\".");
            }
        ...
        /// Keeps the body small; heavy lifting should be inside CTConfig.LoadApply().
        /// </summary>
        [HarmonyPatch]
        static class Patch_Hotkey_ReloadConfig_ChatTranslator
        {
            ...
            {
                if (!CTHotkeys.ReloadPressedThisFrame()) return;
                ...
                try
                {
                    ...
                    SendFeedback($"[CTChat] Config hot-reloaded from \"{PathShortener.ShortenForLog(CTConfig.ConfigPath)}\".");
                }
                catch (Exception ex)
                {
                    SendFeedback($"[CTChat] Hot-reload failed: {ex.Message}.");
                }
            }
        }
        #endregion
        ...
        #region Bypass Guards (Prevent Recursion When We Re-Invoke Originals)

        [ThreadStatic] private static bool _bypass_BroadcastTextMessage_Send;
        [ThreadStatic] private static bool _bypass_CastleMinerZGame_processBroadcastTextMessage;

        private static MethodInfo _mi_processBroadcastTextMessage;

        /// <summary>
        /// Calls BroadcastTextMessage.Send while temporarily bypassing our prefix.
        /// Used by the async translation completion path.
        /// </summary>
        internal static void InvokeBroadcastTextSend_Bypass(LocalNetworkGamer from, string message)
        {
            if (from == null) return;

            try
            {
                _bypass_BroadcastTextMessage_Send = true;
                BroadcastTextMessage.Send(from, message);
            }
            finally
            {
                _bypass_BroadcastTextMessage_Send = false;
            }
        }

        /// <summary>
        /// Calls CastleMinerZGame._processBroadcastTextMessage(message) while temporarily bypassing our prefix.
        /// Used by the async translation completion path.
        /// </summary>
        internal static void InvokeProcessBroadcastTextMessage_Bypass(CastleMinerZGame game, Message message)
        {
            if (game == null || message == null) return;

            try
            {
                if (_mi_processBroadcastTextMessage == null)
                    _mi_processBroadcastTextMessage = AccessTools.Method(typeof(CastleMinerZGame), "_processBroadcastTextMessage");

                _bypass_CastleMinerZGame_processBroadcastTextMessage = true;
                _mi_processBroadcastTextMessage?.Invoke(game, new object[] { message });
            }
            finally
            {
                _bypass_CastleMinerZGame_processBroadcastTextMessage = false;
            }
        }
        #endregion

        #region BroadcastTextMessage.Send - Outgoing Chat Hook

        /// <summary>
        /// Outgoing chat hook. Lets us rewrite the message string before it is
        /// serialized and sent over the network, so we can:
        /// - Send translated text to other players (baseline -> remote language).
        /// - Cache the original baseline text for our own local display.
        /// </summary>
        [HarmonyPatch(typeof(BroadcastTextMessage), "Send")]
        internal static class BroadcastTextMessage_Send_Patch
        {
            static bool Prefix(LocalNetworkGamer from, ref string message)
            {
                if (_bypass_BroadcastTextMessage_Send)
                    return true; // Internal re-send.

                return ChatTranslationState.OnOutgoingSendPrefix(from, ref message);
            }
        }
        #endregion

        #region CastleMinerZGame._processBroadcastTextMessage - Incoming Chat Hook

        /// <summary>
        /// Incoming chat hook. Runs on the receiving side (including our own
        /// client) so we can tag and translate chat messages as they arrive:
        ///   "[ES->EN] Hola..."  or  "[EN->ES] Hello..."
        /// without changing what other, unmodded clients see.
        /// </summary>
        [HarmonyPatch(typeof(CastleMinerZGame), "_processBroadcastTextMessage")]
        internal static class CastleMinerZGame_processBroadcastTextMessage_Patch
        {
            static bool Prefix(CastleMinerZGame __instance, Message message)
            {
                if (_bypass_CastleMinerZGame_processBroadcastTextMessage)
                    return true; // Internal re-invoke.

                return ChatTranslationState.OnIncomingMessagePrefix(__instance, message);
            }
        }
        #endregion

        #region DNAGame.LeaveGame - Reset Translation On Exit

        /// <summary>
        /// When leaving the game, automatically turn off all translation state
        /// (auto mode, manual remote, detected language) so a fresh session
        /// always starts "clean".
        /// </summary>
        [HarmonyPatch(typeof(DNAGame), "LeaveGame")]
        internal static class DNAGame_LeaveGame_Patch
        {
            static void Postfix()
            {
                // Only do anything if translation is actually in use.
                ...
                Log("ChatTranslator: All translation disabled (auto + manual) on LeaveGame().");
            }
        }
        #endregion
