...
namespace TreeFeller
{
    ...
    class GamePatches
    {
        ...
        /// USAGE
        /// -----
        /// TFHotkeys.SetReloadBinding("Ctrl+Shift+F3");
        /// // ... each frame (via Harmony patch) -> if (TFHotkeys.ReloadPressedThisFrame()) { TFConfig.LoadApply(); ... }
        ///
        /// EXAMPLES
        /// --------
        /// "F9"                 -> F9.
        ...
        /// </summary>
        internal static class TFHotkeys
        {
        ...
        static class Patch_Hotkey_ReloadConfig_TreeFeller
        {
            ...
            {
                if (!TFHotkeys.ReloadPressedThisFrame()) return;
                ...
                try
                {
                    ...
                    SendFeedback($"[TFeller] Config hot-reloaded from \"{PathShortener.ShortenForLog(TFConfig.ConfigPath)}\".");
                }
                catch (Exception ex)
                {
        ...
        /// <summary>
        /// Captures the exact block the player began digging so the postfix can evaluate it
        /// after vanilla removes the original trunk block.
        /// </summary>
        ...
        /// <summary>
        /// Triggers tree felling only after an effective axe / chainsaw dig against a log block.
        /// </summary>
        [HarmonyPatch(typeof(InGameHUD), nameof(InGameHUD.Dig))]
        internal static class Patch_InGameHUD_Dig_TreeFeller
        {
            static void Prefix(InGameHUD __instance, InventoryItem tool, bool effective, ref DigContext __state)
            {
                ...
                try
                {
                    ...
                    if (!TreeFellerSystem.IsTreeFellerTool(tool))
                        return;
                    ...
                    if (targetType != BlockTypeEnum.Log)
                        return;
            ...
            static void Postfix(InGameHUD __instance, InventoryItem tool, bool effective, DigContext __state)
            {
                try
                {
                    ...
                    TreeFellerSystem.TryFellTree(__state.Target, tool, localGamer);
                }
                catch (Exception ex)
                {
