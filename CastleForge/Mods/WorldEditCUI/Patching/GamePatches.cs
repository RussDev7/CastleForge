...
namespace WorldEditCUI
{
    ...
    class GamePatches
    {
        ...
        /// </summary>
        internal static class CUIHotkeys
        {
            ...
            public static void SetReloadBinding(string s)
            {
                ...
                Log($"[WECUI] Reload hotkey set to \"{s}\".");
            }
        ...
        static class Patch_Hotkey_ReloadConfig_WorldEdit
        {
            ...
            {
                if (!CUIHotkeys.ReloadPressedThisFrame()) return;
                ...
                try
                {
                    ...
                    SendFeedback($"[WECUI] Config hot-reloaded from \"{PathShortener.ShortenForLog(CUIConfig.ConfigPath)}\".");
                }
                catch (Exception ex)
                {
                    SendFeedback($"[WECUI] Hot-reload failed: {ex.Message}.");
                }
            }
        }
        #endregion
        ...
        #region GameScreen Draw Hook (WorldEditCUI Overlay)

        // This Harmony patch intercepts GameScreen.gameScreen_AfterDraw()
        // to draw the WorldEditCUI selection box directly into the game's 3D scene.
        [HarmonyPatch]
        internal static class GameScreen_AfterDraw_CUIPatch
        {
            /// <summary>
            /// Fast accessor for GameScreen's private "mainView" field.
            /// We pull matrices off the live CameraView each frame.
            /// </summary>
            private static readonly AccessTools.FieldRef<GameScreen, CameraView> _mainViewRef =
                AccessTools.FieldRefAccess<GameScreen, CameraView>("mainView");

            /// <summary>
            /// Targets: void GameScreen.gameScreen_AfterDraw(object sender, DrawEventArgs e)
            /// </summary>
            static MethodBase TargetMethod()
                => AccessTools.Method(typeof(GameScreen), "gameScreen_AfterDraw",
                    new[] { typeof(object), typeof(DrawEventArgs) });

            /// <summary>
            /// Postfix runs AFTER the game's own AfterDraw routine.
            /// We draw our selection outline using our BasicEffect-backed renderer (no SpriteBatch required).
            /// </summary>
            static void Postfix(GameScreen __instance, object sender, DrawEventArgs e)
            {
                try
                {
                    // Guard: Only draw when WorldEdit's selection overlay is enabled.
                    if (!WorldEdit.WorldEditCore._enableCLU) return;

                    // Fetch the active camera view and camera.
                    var mv  = _mainViewRef(__instance);
                    var cam = mv?.Camera;
                    if (cam == null)
                        return;

                    // Pull camera matrices for world->screen transforms.
                    Matrix view = cam.View;
                    Matrix proj = cam.GetProjection(e.Device);

                    // WorldEdit selection points (public statics in WorldEditCore).
                    var p1 = WorldEdit.WorldEditCore._pointToLocation1;
                    var p2 = WorldEdit.WorldEditCore._pointToLocation2;

                    // Draw selection + optional chunk overlays.
                    CUIOverlayRenderer.DrawSelectionOverlay(e.Device, view, proj, p1, p2);
                }
                catch (Exception ex)
                {
                    // Never throw from draw hooks; log and keep the frame alive.
                    try { Log($"[Patch] gameScreen_AfterDraw Postfix failed: {ex}."); } catch { }
                }
            }
        }
        #endregion
